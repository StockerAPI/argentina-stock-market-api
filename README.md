# Argentina stock market API

Real-time Argentina stock market API with WebSocket, OHLCV, and snapshot endpoints for BYMA.

## Build With Argentina Market Data

Kun Data provides a token-authenticated API stack for developers who need Argentina market data in products that must load fast, stream live prices, and support charting or screening workflows.

This repository is focused on Argentina coverage with support for BYMA and is designed for teams building broker dashboards, watchlists, quant tools, stock screeners, mobile apps, and market overview pages.

## Why Developers Search For This

Developers usually need one or more of these:

- Real-time Argentina stock market API
...
`Argentina stock market API | BYMA API | Argentina stock data API | Argentina real-time stock API`
...
### Does this support real-time Argentina market data?
...
If you need Argentina market data coverage for BYMA, start with the product page and documentation:
